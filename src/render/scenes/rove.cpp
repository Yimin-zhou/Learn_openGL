...
void RoverRender::init()
{
	...
	// light
	{
		std::shared_ptr<DirectionalLight> directionalLight = std::make_shared<DirectionalLight>(glm::vec3(5.0f, -1.0f, 1.0f), glm::vec3(1.0f, 1.0f, 1.0f), 3.0f);
...
{
	// Render glass
	std::shared_ptr<Shader> shader = m_shaderManager.getShader(ShaderName::GBUFFER);
	shader->bind();
